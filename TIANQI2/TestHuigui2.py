...
xTrain = np.array(lows,dtype=np.int)[:, np.newaxis]  # 训练数据（日期）
yTrain = np.array(highs,dtype=np.int)                # 训练数据（最高气温）
xTest = np.array([17,18,16,21])[:,np.newaxis]            # 测试数据（日期）
yTest = np.array([25,26,21,28])                      # 测试数据（最高气温）
# 创建模型对象
